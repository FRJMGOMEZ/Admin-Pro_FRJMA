...
export class LoginComponent implements OnInit {
  ...
  constructor(public router:Router, public headerTitle:Title,private meta:Meta, public _userService:UserService) {

     this.getDataRoute().subscribe(event => {
       this.headerTitle.setTitle(event.title);
       const metaTag: MetaDefinition = { name: event.description, content: event.title};
  ...
  googleInit(){
    ...
    gapi.load('auth2',()=>{
      this.auth2 = gapi.auth2.init({
        clien_id:'97587997178-d4m3g8lhl9g3flctnvmpmdonk5tgo9c7.apps.googleusercontent.com',
